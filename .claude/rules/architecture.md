...
| `platform.ts` | `isWindows`, `claudeJsonPath`, `vaultsRoot`, `findTool`, `npmGlobalBin`, `installGhForPlatform` (winget/brew/apt/dnf bootstrap for `gh`; throws `VaultkitError('TOOL_MISSING')` on unsupported platforms — used by `setup` and `init` when `findTool('gh')` returns null), `getLauncherTemplate()` / `getDeployTemplate()` (single source of truth for the byte-immutable template paths under `lib/`; resolves to `<repo>/lib/...` in dev and `<install>/dist/lib/...` after build because the post-build script keeps the relative offset constant — used by `init`, `update`, `visibility`) |
